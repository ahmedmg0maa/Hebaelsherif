...
كانت أزرار إدارة الحجوزات غير قادرة على تحديث الحالات غير `confirmed` لأن كود التحديث كان يرسل قيمة `undefined` داخل `updateDoc` إلى Supabase:

```ts
paymentStatus: status === 'confirmed' ? 'confirmed' : undefined
```

Supabase لا يقبل `undefined` في بيانات الكتابة افتراضيًا، لذلك كان زر **تأكيد الحجز** يعمل فقط لأنه يرسل قيمة صريحة، بينما أزرار **بانتظار مراجعة الدفع** و **تحديد كمكتملة** و **إلغاء الحجز** تفشل قبل تنفيذ الكتابة.
