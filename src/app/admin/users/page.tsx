...
import { AdminActionButton, AdminPageHeader, AdminPanel, EmptyState, Field, inputClass, MetricCard, ToneBadge } from '@/components/admin/OperationsUI'
...
export default function AdminUsersPage() {
  ...
  async function load() {
    ...
    try {
      ...
      setUsers(mapDocs<UserItem>(usersSnap).sort((a, b) => toMillis(b.createdAt) - toMillis(a.createdAt)))
  ...
  async function setRole(user: UserItem, role: 'user' | 'admin') {
    if (!window.confirm(`تغيير دور ${user.email} إلى ${role === 'admin' ? 'أدمن' : 'مستخدم'}؟`)) return
    ...
    try {
      ...
      await addDoc(collection(db, 'admin_logs'), {
        ...
        after: { role },
        message: `تغيير دور المستخدم ${user.email}`,
      ...
      setError('تعذر تحديث الدور. تأكد من الصلاحيات.')
    } finally {
      setSavingId('')
    }
  }
  ...
  const userMetrics = useMemo(() => {
    const byUser = users.map((user) => {
      ...
      const paidTotal = userOrders.filter(isPaidOrder).reduce((sum, order) => sum + getAmount(order), 0)
  ...
  const filtered = useMemo(() => {
    ...
    return userMetrics.filter(({ user, orders: userOrders, bookings: userBookings, paidTotal }) => {
      const matchesSearch = !query || [user.name, user.displayName, user.email, user.phone, user.uid].filter(Boolean).join(' ').toLowerCase().includes(query)
      const matchesFilter =
        filter === 'all' ||
  ...
  return (
    <div className="space-y-8">
      <AdminPageHeader title="العملاء والمستخدمون" description="فهم العملاء من زاوية الطلبات والحجوزات والإيرادات، مع تحكم حذر في أدوار المستخدمين." />
      ...
      <AdminPanel title="فلترة العملاء" description="ابحث عن عميلة وشاهد طلباتها وحجوزاتها بسرعة.">
        <div className="grid gap-4 md:grid-cols-3">
          ...
          <div className="md:col-span-2">
            <Field label="بحث">
              <input className={inputClass} value={search} onChange={(event) => setSearch(event.target.value)} placeholder="اسم، بريد، هاتف..." />
            </Field>
          </div>
        </div>
      </AdminPanel>

      <AdminPanel title="قائمة العملاء" description="البيانات المعروضة محسوبة من الطلبات والحجوزات الحقيقية.">
        {filtered.length === 0 ? <EmptyState title="لا توجد نتائج" description="غير الفلتر أو البحث لعرض العملاء." /> : (
          <div className="grid gap-4 xl:grid-cols-2">
            {filtered.map(({ user, orders: userOrders, bookings: userBookings, paidTotal }) => (
              <article key={user.uid} className="rounded-[1.75rem] border border-sand bg-cream/80 p-5 shadow-soft dark:border-gold/25 dark:bg-white/10">
                <div className="mb-4 flex flex-wrap items-center gap-3">
                  <h3 className="text-lg font-black text-charcoal dark:text-ivory">{user.name || user.displayName || user.email}</h3>
                  <ToneBadge tone={user.role === 'admin' ? 'gold' : 'muted'}>{user.role === 'admin' ? 'Admin' : 'User'}</ToneBadge>
                ...
                <div className="grid gap-3 text-sm font-bold text-warm-gray dark:text-cream md:grid-cols-2">
                  <p>البريد: <span className="text-charcoal dark:text-ivory">{user.email}</span></p>
                  <p>الهاتف: <span className="text-charcoal dark:text-ivory">{user.phone || 'غير متوفر'}</span></p>
                  <p>الطلبات: <span className="text-charcoal dark:text-ivory">{formatNumber(userOrders.length)}</span></p>
                  <p>الحجوزات: <span className="text-charcoal dark:text-ivory">{formatNumber(userBookings.length)}</span></p>
                  <p>إجمالي المدفوع: <span className="text-charcoal dark:text-ivory">{formatEGP(paidTotal)}</span></p>
                  <p>التسجيل: <span className="text-charcoal dark:text-ivory">{formatArabicDateTime(user.createdAt)}</span></p>
                </div>
                <div className="mt-5 flex flex-wrap gap-2">
                  {user.role === 'admin' ? (
                    <AdminActionButton disabled={savingId === user.uid} tone="warning" onClick={() => setRole(user, 'user')}>إزالة Admin</AdminActionButton>
                  ) : (
                    <AdminActionButton disabled={savingId === user.uid} tone="gold" onClick={() => setRole(user, 'admin')}>تعيين Admin</AdminActionButton>
                  )}
                </div>
              </article>
            ))}
