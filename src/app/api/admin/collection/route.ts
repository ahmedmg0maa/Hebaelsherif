...
export async function GET(req: NextRequest) {
  try {
    ...
    if (!collectionName) return NextResponse.json({ error: 'Invalid collection.' }, { status: 400 })
...
export async function POST(req: NextRequest) {
  try {
    ...
    const body = (await req.json()) as { id?: unknown; values?: Record<string, unknown>; action?: unknown }
    ...
    if (id) {
      await db.collection(collectionName).doc(id).set({ ...values, updatedAt: Timestamp.now(), updatedBy: adminId }, { merge: true })
