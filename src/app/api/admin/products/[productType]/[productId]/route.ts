...
export async function POST(req: NextRequest, { params }: { params: Promise<{ productType: string; productId: string }> }) {
  const { productType: rawProductType, productId: rawProductId } = await params
  try {
    const { error, context } = await requireAdmin(req, ['owner', 'admin', 'super_admin', 'content_manager'])
    if (error || !context) return error

    const productType = cleanString(rawProductType)
