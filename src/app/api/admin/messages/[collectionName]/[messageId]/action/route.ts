...
interface RouteContext {
  params: Promise<{ collectionName: string; messageId: string }>
}
...
export async function POST(req: NextRequest, context: RouteContext) {
  try {
    ...
    const collectionName = cleanText((await context.params).collectionName, 120)
    const messageId = cleanText((await context.params).messageId, 160)
