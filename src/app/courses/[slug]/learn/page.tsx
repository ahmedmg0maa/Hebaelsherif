...
export default function CourseLearningPage() {
  ...
  async function markComplete() {
    if (!sessionUser || !course || !activeLesson) return
    try {
      const token = await sessionUser.getIdToken()
        ...
        {!loading && (!course || error) ? (
          <section className="container-premium py-12">
            <PremiumEmptyState
              ...
              actionLabel={sessionUser ? 'العودة لكورساتي' : 'تسجيل الدخول'}
              actionHref={sessionUser ? '/dashboard/courses' : `/auth/login?next=${encodeURIComponent(`/courses/${slug}/learn`)}`}
            />
          </section>
        ) : null}
