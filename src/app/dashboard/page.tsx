...
export default function DashboardHomePage() {
  ...
  useEffect(() => {
    ...
    async function loadDashboardStats() {
      ...
      setStats({
        paidCourses: orders.filter(
          (order) => order.productType === 'course' && order.status === 'paid',
        ).length,
        paidBooks: orders.filter((order) => order.productType === 'book' && order.status === 'paid')
          .length,
  ...
  return (
    <div>
      <div className="mb-8">
        <p className="mb-2 text-sm font-bold text-gold">لوحة رحلتك</p>
        <h2 className="text-3xl font-black text-charcoal">كل ما يخصك في مكان واحد</h2>

        <p className="mt-3 max-w-2xl text-sm leading-8 text-warm-gray">
          تابعي الدورات والكتب والجلسات والطلبات الخاصة بكِ من هنا.
        </p>
      </div>
      ...
      {!hasAnyActivity ? (
        <div className="mt-8">
          <PremiumEmptyState
            ...
            description="عند شراء دورة أو كتاب أو حجز جلسة، سيظهر كل شيء هنا تلقائياً."
