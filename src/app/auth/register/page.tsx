...
import { AuthCompatError } from '@/lib/supabase/auth-error-compat'
...
function getArabicAuthCompatError(error: unknown) {
  if (!(error instanceof AuthCompatError)) {
  ...
  const messages: Record<string, string> = {
    ...
    'auth/operation-not-allowed': 'طريقة التسجيل غير مفعلة من Supabase.',
...
function RegisterPageContent() {
  ...
  async function handleRegister(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setError(getArabicAuthCompatError(registerError))
    } finally {
  ...
  async function handleGoogleLogin() {
    ...
    try {
      ...
      setError(getArabicAuthCompatError(googleError))
    } finally {
