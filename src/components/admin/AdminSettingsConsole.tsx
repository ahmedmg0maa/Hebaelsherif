...
export default function AdminSettingsConsole({
  ...
  useEffect(() => {
    async function loadSettings() {
      ...
      try {
        ...
        if (!response.ok) {
          setError('تم فتح القيم الافتراضية مؤقتًا. راجع صلاحيات الأدمن أو Supabase Admin إذا لم يتم الحفظ.')
        }
      } catch (loadError) {
        console.error('Admin settings load error:', loadError)
        setValues(defaults)
        setError('تم فتح القيم الافتراضية مؤقتًا. يمكنك الحفظ بعد التأكد من اتصال Supabase Admin.')
      } finally {
        setLoading(false)
      }
    }
    ...
    loadSettings()
  }, [collectionName, documentId, fields, sessionUser])
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!response.ok) {
        setError('تعذر حفظ الإعدادات. راجع صلاحيات الأدمن أو متغيرات Supabase Admin.')
        return
      }
