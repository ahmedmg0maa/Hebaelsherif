    ...
    <div className="relative space-y-4">
      ...
      {items.map((item, index) => (
        <div key={item.title} className="relative pr-14">
          <span className="absolute right-0 top-1 flex h-10 w-10 items-center justify-center rounded-full border border-sand bg-burgundy text-xs font-black text-ivory shadow-soft">
            {index + 1}
          </span>
